...
export const triggers = {
    ...
    'LOG:FOUND:PROFILE': createRegExp(oneOrMore(digit)
    ...
    'LOG:LOBBY:POPULATING:MAP': createRegExp(exactly('GAME -- *** Beginning mission ')
        .and(oneOrMore(char).before(' (').groupedAs('map'))),
    'LOG:LOBBY:POPULATING:PLAYER': createRegExp(oneOrMore(digit)
    ...
    'LOG:LOBBY:POPULATING:PLAYER:COUNT': createRegExp(oneOrMore(digit).after('GetMaxFrameTimeFromProfile: players=').groupedAs('count')),
    'LOG:LOBBY:POPULATING:PLAYER:STEAM': createRegExp(exactly('/steam/')
        .and(oneOrMore(digit).groupedAs('steamId'))
        .and(oneOrMore(char))
        .and(digit.after('slot =  ').groupedAs('slot'))
        .and(oneOrMore(char))
        .and(exactly('ranking = '), oneOrMore(whitespace), oneOrMore(digit).or('-1').groupedAs('ranking'))),
    'LOG:LOBBY:POPULATING:MATCH:TYPE': createRegExp(exactly('AutoMatchForm - Starting game')
 ...
 * @param matcher
 */
export function addEvent(name, matcher) {
